...
CLOUD_TEMPLATE_YAML_FILE="/opt/StreamAnalytix/conf/yaml/templates/azure-template/cloud-conf-template.yaml"
...
sed -ri 's|^(\s*)(password\s*:\s*"admin"\s*$)|\1password: '"$HDI_ADMIN_PWD"'|' $CLOUD_TEMPLATE_YAML_FILE
sed -ri 's|^(\s*)(clustername\s*:\s*""\s*$)|\1clustername: '"$CLUSTER_NAME"'|' $CLOUD_TEMPLATE_YAML_FILE

echo "-------------------------------------HOST NAMES-------------------------------------------"
## get HDInsights Cluster Host Names & update /etc/hosts file 
HOST_NAMES=$(curl -sS -G -u $HDI_CREDS $HDINSIGHT_URLBASE/hosts \ | jq '.host_components[].HostRoles.host_name')
echo "CLUSTER_HOST_NAME : $HOST_NAMES"

for HOSTNAME in $(curl -sS -G -u $HDI_CREDS $HDINSIGHT_URLBASE/hosts \ | jq '.host_components[].HostRoles.host_name')
do
    IP=$(curl -sS -G -u $HDI_CREDS "$HDINSIGHT_URLBASE/hosts/$HOSTNAME" | jq -r '.Hosts.ip')
	
    	echo "$HOSTNAME <--> $IP"
	sed -i "$IP  $HOSTNAME" /etc/hosts
	if  [[ $HOSTNAME == hn0* ]] 
	then
	    PRIMARY_HEAD_NODE=$HOSTNAME
	    echo "PRIMARY_HEAD_NODE : $PRIMARY_HEAD_NODE"
	fi

	if  [[ $HOSTNAME == hn1* ]] 
	then
	    SECONDARY_HEAD_NODE=$HOSTNAME
...
YARN_RESOURCE_MANAGER_HOSTS=$(curl -sS -G -u $HDI_CREDS $HDINSIGHT_URLBASE/services/YARN/components/RESOURCEMANAGER \ | jq '.host_components | map(.HostRoles.host_name) | join("##")')

echo "YARN_RESOURCE_MANAGER_HOSTS : $YARN_RESOURCE_MANAGER_HOSTS"
