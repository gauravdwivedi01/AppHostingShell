...
SAX_CONF_SHELL_NAME="sax-yaml-configure-v0.sh"
SAX_MYSQL_JAR_NAME="mysql-connector-java-5.1.30.jar"
JQ_ZIP_NAME="jq.zip"
SAX_DOWNLOAD_BUNDLE_PATH="$SAX_HOME$SAX_DOWNLOAD_JAR_NAME"
SAX_DEPENDENCY_SHELL_PATH="$SAX_HOME$SAX_DEPENDENCY_SHELL_NAME"
SAX_MYSQL_JAR_PATH="$SAX_HOME$SAX_MYSQL_JAR_NAME"
...
if [ -f "$SAX_DOWNLOAD_BUNDLE_PATH" ]
	...
	echo "Successfully moved StreamAnalytix.war jar to server/tomcat/webapps !!"
        ...
        #echo "Moving mysql-connector jar to conf & tomcat lib !!"
	#retry 3 wget -O "$SAX_MYSQL_JAR_NAME" "$SAX_MYSQL_DOWNLOAD_LINK"
	#cp mysql-connector-java-5.1.30.jar $SAX_INSTALLATION_DIR/conf/thirdpartylib/
	#cp mysql-connector-java-5.1.30.jar $SAX_INSTALLATION_DIR/server/tomcat/lib/
	#cp mysql-connector-java-5.1.30.jar $SAX_HOME
	#echo "Successfully moved mysql-connector jar to conf & tomcat lib !!"
...
chmod 755 sax-dependencies-install-v0.sh
...
echo "Current StreamAnalytix directory is -- $PWD"
...
#starting server in embedded mode
